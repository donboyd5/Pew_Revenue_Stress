...
moody <- read_excel(here::here("ignore", "pew", mfn), 
                    sheet="GDP", range="A6:C421", col_names = c("yq", "moodys50", "moodys90"))
gdp_moody_jul2020 <- moody %>%
  mutate(year=str_sub(yq, 1, 4) %>% as.numeric,
         q=str_sub(yq, -1, -1) %>% as.numeric,
         year=ifelse(q >= 3, year + 1, year)) %>%
  group_by(year) %>%
...
```{r}
gdp_master %>%
  left_join(gdp_moody_jul2020, by="year")


```


## National GDP forecasts from Moody's December 2020 "Moody's GSP December 2020.xlsx"

*  "Moody's GSP December 2020.xlsx"
*  construct US as sum of the 50 states plus DC
...
df3 <- df2 %>%
  ...
  summarise(value=mean(value) * 1000, .groups = "drop") # put in $ millions
summary(df3)
count(df3, year)
...
gdp_master %>%
  ...
  left_join(gdp_moody_jul2020, by="year") %>%
  left_join(gdp_moody_dec2020, by="year")
...
usgdp1 <- gdphist %>%
  ...
  full_join(gdp_moody_jul2020, by="year") %>%
  full_join(gdp_moody_dec2020, by="year")
...
nontaxgrowth <- stgdp %>%
  select(year, stabbr, moodys_dec2020) %>%
  filter(year %in% 2018:2020) %>%
  mutate(ratio=moodys_dec2020 / moodys_dec2020[year==2018],
         name="nontax") %>%
  select(year, stabbr, name, ratio)
