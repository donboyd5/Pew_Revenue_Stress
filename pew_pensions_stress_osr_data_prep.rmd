...
## GDP history and forecasts

### Get Moody's latest national and state forecasts

```{r ONETIME_get_moodysdec2020_from_Kimberly, eval=FALSE}

kbdir <- here::here("ignore", "Kimberly")

# 50 states gdp
df <- read_excel(here::here(kbdir, "Moody's GSP December 2020.xlsx"))
...
saveRDS(dfl, here::here("data", "gdpstus_moodys_dec2020.rds"))
...
## National GDP: get and splice history and forecasts
...
```{r ONETIME_usgdp_forecasts, eval=FALSE}
# get Moody's, CBO, fedalt gdp forecasts and splice
...
moody2 <- moody %>%
  mutate(year=str_sub(yq, 1, 4) %>% as.numeric,
         ...
         year=ifelse(q >= 3, year + 1, year)) %>%
  group_by(year) %>%
  summarise(moodys_jul2020p50=mean(moodys50),
            moodys_jul2020p90=mean(moodys90),
            .groups="drop") %>%
  pivot_longer(-year, names_to = "src") %>%
  mutate(stabbr="US",
         name="gdp",
         fullname=paste0(name, "_", src))
summary(moody2) # amounts are in billions

# this second file has forecasts only; I believe it was prepared by Kimberly Burham
# the Moody's baseline appears to be the same as in the first file
...
# moodys_jul2020
gdp <- read_excel(here::here("ignore", "NJ", gdpfn), sheet="GDP", 
...
# now get real also, in case we need it
sfy <- read_excel(here::here("ignore", "NJ", gdpfn), sheet="Real GDP", range="A3:A14", col_names = "year")
# CBO pre-COVID	CBO July	Moody's 50th July	Lena Fed W	Fed Alt Severe
# moodys_jul2020
rgdp <- read_excel(here::here("ignore", "NJ", gdpfn), sheet="Real GDP", 
                   range="G3:K14", col_names=vnames)
# keep only the FY values, not the Q2 values
rgdp2 <- tibble(sfy, rgdp) %>%
  mutate(stabbr="US",
         name="rgdp") %>%
  pivot_longer(-c(year, name, stabbr), names_to = "src") %>%
  mutate(fullname=paste0(name, "_", src))
...
# check whether nominal gdp is the same in the Moody's file and the second file
check <- bind_rows(moody2 %>% filter(src == "moodys_jul2020p50") %>% mutate(file="mfile"),
                   gdp2  %>% filter(src == "moodys_jul2020p50") %>% mutate(file="gfile")) %>%
  select(year, file, value) %>%
  pivot_wider(names_from = file) %>%
  filter(!is.na(gfile)) %>%
  mutate(diff=gfile - mfile)
check

# get Moody's Dec 2020 baseline forecast from previously created file from Kimberly
# make src="moodys_dec2020p50"
#  year stabbr name src value fullname
moody_dec2020 <- readRDS(here::here("data", "gdpstus_moodys_dec2020.rds")) %>%
  filter(name=="us", stabbr=="NY", year>=2019, year<=2030) %>%  # any stabbr will do
  mutate(src="moodys_dec2020p50",
         name="gdp",
         stabbr="US",
         fullname=paste0(name, "_", src))


# get the p90 forecast from the moody's file
stack <- bind_rows(gdp2, 
                   rgdp2, 
                   moody2 %>% 
                     filter(src == "moodys_jul2020p90", year>=2019, year<=2030),
                   moody_dec2020) %>%
  filter(!is.na(value))
summary(stack) # this is in billions so we don't need to change scale

saveRDS(stack, here::here("data", "usgdp_forecast.rds"))
rm(mfn, moody, moody2, gdp, gdp2, rgdp, rgdp2, check, stack, sfy, moody_dec2020)

# tmp <- readRDS(here::here("data", "usgdp_forecast.rds"))
# summary(tmp)
# count(tmp, src)
# tmp %>% filter(src=="moodys_jul2020p50")
# tmp %>%
#   filter(name=="gdp") %>%
...
# 2 cbo_precovid         24
# 3 fed_alt              24
# 4 moodys_dec2020p50    12
# 5 moodys_jul2020p50    24
# 6 moodys_jul2020p90    12
...
splice_hf <- function(history, forecast, firstyear=NULL){
  ...
  df <- tibble(year=min(history$year):max(forecast$year)) %>%
    ...
    mutate(src=forecast$src[1],
           ratio=case_when(year < firstyear ~ 1,
                           year == firstyear ~ 1,
  ...
  df %>%
    select(year, h, f, value, src)
}

```


```{r ONETIME_splice_usgdp}
ushist <- readRDS(here::here("data", "usgdp_history.rds"))
usfc <- readRDS(here::here("data", "usgdp_forecast.rds")) %>%
  filter(name=="gdp", stabbr=="US") %>%
  mutate(value=value * 1000) %>%  # put in $ millions
  select(year, src, value)

summary(ushist) # 1947-2020
summary(usfc)
count(usfc, name)  # gdp, rgdp
count(usfc, src)
...
f <- function(src){
  gdpfc <- usfc %>%
    filter(src==!!src)
  splice_hf(gdph, gdpfc) %>%
    select(year, value, src) %>%
    mutate(name="gdp")
}

usgdp_spliced <- map_dfr(unique(usfc$src), f)

saveRDS(usgdp_spliced, here::here("data", "usgdp_spliced.rds"))
...
usgdp_spliced <- readRDS(here::here("data", "usgdp_spliced.rds"))
usgdp_spliced %>%
  filter(year >= 2015) %>%
  ggplot(aes(year, value, colour=src)) +
  geom_line() +
  geom_point() +
  ggtitle("National forecasts, nominal GDP, state fiscal year basis")

```


## State GDP: get history and construct forecasts using Moody's national-state differences


```{r ONETIME_stgdp_history, eval=FALSE}
# year, name, value, stabbr, src, fullname

# bea history from quarterly back to 2005, convert to sfy
dfq <- sgdp.q %>%
...
dfa <- sgdp_spliced.a %>%
  ...
  ungroup() %>%
  filter(!is.na(value)) %>%
  select(year, stabbr, value, name, src, fullname)
...
dfa2 <- dfa %>%
  ...
  mutate(r2005=value / value[year==2005]) %>%  # ratio vs 2005
  ungroup

dfall <- dfq %>%
  select(year, stabbr, name, val.q=value) %>%
  right_join(dfa2 %>% select(year, stabbr, name, r2005, val.a=value),
            by=c("year", "stabbr", "name")) %>%
  arrange(name, year) %>%
  group_by(stabbr, name) %>%
  mutate(value=ifelse(year >= 2005, val.q, val.q[year==2005]*r2005),
         src="beaspliced",
         fullname=paste0(name, "_", src)) %>%
  ungroup %>%
  filter(!is.na(value))
...
# looks good, save
dfsave <- dfall %>%
  select(year, name, value, stabbr, src, fullname)
...
```{r ONETIME_splice_stgdp, eval=FALSE}
# Dec 2020 Moody's state gdp forecast
# saveRDS(dfsave, here::here("data", "stgdp_history.rds"))

usgdp_spliced <- readRDS(here::here("data", "usgdp_spliced.rds"))

sthist <- readRDS(here::here("data", "stgdp_history.rds")) %>%
  filter(name=="gdp")
summary(sthist)  # 1964-2019

histend <- 2019

# get the delta in pct change vs US
stpdelta <- readRDS(here::here("data", "gdpstus_moodys_dec2020.rds")) %>%
  filter(name=="pdelta", year >= 2019, year <= 2030) %>%
  select(stabbr, year, pdelta=value)
summary(stpdelta)

# now get the % change for each state under each US forecast
stpch <- usgdp_spliced %>%
  arrange(src, year) %>%  
  group_by(src) %>%
  mutate(pch= value / value[match(year - 1, year)] * 100 -100) %>%
  ungroup %>%
  filter(year >= histend) %>%
  left_join(stpdelta, by=c("year")) %>%
  mutate(stpch=pch + pdelta) %>%
  # compute the ratio of each state's value to its value in 2019
  group_by(src, stabbr) %>%
  arrange(year) %>%
  mutate(ipch=ifelse(year==2019, 1, 1 + stpch / 100),
         r2019=cumprod(ipch)) %>%
  ungroup %>%
  arrange(src, stabbr, year)
stpch  

# get the state gdp forecasts
stfc <- stpch %>%
  select(year, stabbr, src, r2019) %>%
  left_join(sthist %>% filter(year==2019, name=="gdp") %>% select(stabbr, basevalue=value),
            by="stabbr") %>%
  mutate(value=basevalue * r2019) %>%
  select(year, src, stabbr, value)
...
stgdp_spliced <- map_dfr(unique(stfc$src), f)

stgdp_spliced %>%
  filter(year >= 2015, stabbr=="NJ") %>%
  ggplot(aes(year, value, colour=src)) +
  geom_line() +
  geom_point()

saveRDS(stgdp_spliced, here::here("data", "stgdp_spliced.rds"))
...
data %>%
  filter(year >= 2000, year <= 2030, stabbr %in% c("MA", "NJ")) %>%
  ggplot(aes(year, value)) +
  geom_line(colour="blue") +
  geom_point(colour="blue") +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 2020.5, linetype="dotted") +
  geom_hline(aes(yintercept=avg), colour="red", size=0.4, data=avgs %>% filter( stabbr %in% c("MA", "NJ"))) +
  scale_y_continuous(name="State % change minus US % change", breaks=seq(-10, 10, 1)) +
  scale_x_continuous(name="State fiscal year", breaks=seq(1980, 2050, 5)) +
  facet_wrap(~stabbr, ncol=2, scales="fixed") +
  ggtitle("Nominal GDP growth: State minus nation, selected states, Moody's December 2020 forecast",
...
taxgrowth <- df %>%
  filter(year >= 2018) %>%
  group_by(stabbr, name) %>%
  mutate(ratio=value / value[year==2018]) %>%
  ungroup %>%
...
```{r ONETIME_Moodysgdp_nontaxgrowth_for20192020}
# for nontax, use 2019 and 2020 gdp growth unless we get other data

nontaxgrowth <- readRDS(here::here("data", "gdpstus_moodys_dec2020.rds")) %>%
  filter(year %in% 2018:2020, name=="value") %>%
  mutate(ratio=value / value[year==2018],
         name="nontax") %>%
  select(year, stabbr, name, ratio)
...
osrupdate <- bind_rows(osrhist, 
                       growth %>% filter(year %in% 2019:2020)) %>%
  group_by(stabbr, name) %>%
  arrange(year) %>%
  mutate(value=ifelse(year %in% 2019:2020, 
                      value[year==2018] * ratio,
                      value)) %>%
  ungroup %>%
...
```{r OLD_ONETIME_njgdp_forecast, eval=FALSE}
# we have the following data files:
#  Moodys July 2020 State GSP.xlsx -- July baseline, 50 states, DC, PR, NO US, 1977q1, 2050q4, $ billions, nominal
#  Moody's NJ GSP forecasts.xlsx -- two tabs, Dec 2019 and July 2020 baseline, NJ only,  1977q1, 2050q4, $ billions, nominal
# (meshes w/50 states, DC, PR)
...
df2
ht(df2) # good, it is in billions

saveRDS(df2, here::here("data", "njgdp_forecast.rds"))
rm(fn, s1, s2, df, df2)
...
```{r ONETIME_njtax_history, eval=FALSE}
...
saveRDS(dfl, here::here("data", "njtax_history.rds"))
...
fn <- "assumptions.xlsx"
df <- read_excel(here::here("data", fn), sheet="osr_forecasts")
...
saveRDS(njtax_forecast, here::here("data", "njtax_forecast.rds"))

rm(fn, df, njtax_forecast)


```


```{r OLD_ONETIME_combine_data}
# get previously constructed data
# put in $ millions
# year, stabbr, name, src, fullname, value

usgdp_history <- readRDS(here::here("data", "usgdp_history.rds"))
summary(usgdp_history)  # 1947-2020, millions
usgdp_history2 <- usgdp_history

usgdp_forecast <- readRDS(here::here("data", "usgdp_forecast.rds"))
summary(usgdp_forecast) # 2019-2051, billions
usgdp_forecast2 <- usgdp_forecast %>%
  mutate(value=value * 1e3) # put in $ millions

njgdp_history <- readRDS(here::here("data", "njgdp_history.rds"))
summary(njgdp_history) # 1964-2020, millions nominal; later for real
njgdp_history2 <- njgdp_history

njgdp_forecast <- readRDS(here::here("data", "njgdp_forecast.rds"))
summary(njgdp_forecast) # 2020-2051, billions
njgdp_forecast2 <- njgdp_forecast %>%
  mutate(value=value * 1e3) # put in $ millions
count(njgdp_forecast2, fullname)
#   fullname               n
#   <chr>              <int>
# 1 gdp_moodys_dec2019    32
# 2 gdp_moodys_jul2020    32

osrcen <- readRDS(here::here("data", "osr_census_updated.rds"))
summary(osrcen) # 1942-2020, thousands
count(osrcen, fullname)
count(osrcen, stabbr) # just NJ
#   <chr>           <int>
# 1 cit_census         63
# 2 gst_census         63
# 3 iit_census         63
...
osrcen2 <- osrcen %>%
  mutate(value=value / 1e3) # put in millions
osrcen2 %>% filter(year==2020)

njtax_history <- readRDS(here::here("data", "njtax_history.rds"))
summary(njtax_history) # 2010-2019, millions
njtax_history2 <- njtax_history
count(njtax_history, fullname)
#   fullname               n
#   <chr>              <int>
# 1 cit_NJCAFRs           10
# 2 iit_NJCAFRs           10
...
njtax_forecast <- readRDS(here::here("data", "njtax_forecast.rds")) # nominal $ thousands
summary(njtax_forecast) # 2019-2021
njtax_forecast2 <- njtax_forecast %>%
  mutate(value=value / 1e3) # millions

uscg <- readRDS(here::here("data", "uscapgains.rds")) # capgains_CBO_sep2020 and capgains_lagged_CBO_sep2020, nominal billions
summary(uscg) # 1995-2030 one NA, for lagged
uscg2 <- uscg %>%
  mutate(value=value * 1e3) # millions
...
# combine economic and fiscal variables
combo <- bind_rows(usgdp_history2,
                   usgdp_forecast2,
                   njgdp_history2,
                   njgdp_forecast2,
                   uscg2,
...
rm(usgdp_history, usgdp_history2, 
   usgdp_forecast, usgdp_forecast2,
   njgdp_history, njgdp_history2,
   njgdp_forecast, njgdp_forecast2,
   uscg, uscg2, 
   stcg, stcg2,
   osrcen, osrcen2, 
   njtax_history, njtax_history2, 
   njtax_forecast, njtax_forecast2)

```


## ONETIME - splice history and forecasts
...
# note that by Pew choice, cbo_jul2020 and fed_alt have the same growth rates for 2024 and later
combotab
...
gdpmoody90 <- combo %>%
  filter(name=="gdp", stabbr=="US", src=="moodys_jul2020p90") %>%
  select(year, value, src)

usgdp_spliced <- bind_rows(splice_hf(gdph, gdpprior),
                         splice_hf(gdph, gdpbase),
                         splice_hf(gdph, gdpfedalt),
                         splice_hf(gdph, gdpmoody50),
                         splice_hf(gdph, gdpmoody90))
...
## Splice NJ GDP data
```{r}
# for NJ, get the diff in growth rates between Moody's baseline US and baseline NJ and use that to create
# diffs vs national for any national forecast

# get moody's nj diffs
# first, get spliced values for nation and nj

combotab

njh <- combo %>%
  filter(name=="gdp", stabbr=="NJ", src=="beaspliced") %>%
  select(year, value, src)

njfc <- combo %>%
  filter(name=="gdp", stabbr=="NJ", src=="moodys_jul2020") %>%
  select(year, value, src)

njsplice <- splice_hf(njh, njfc)
ussplice <- usgdp_spliced %>%
  filter(src=="moodys_jul2020p50", year >= 1964) # NJ starts in 1964 so only keep US since then
summary(njsplice)
summary(ussplice)
start_year <- min(njsplice$year, ussplice$year)

njus <- bind_rows(njsplice %>% select(year, value) %>% mutate(stabbr="NJ"),
                  ussplice %>% select(year, value) %>% mutate(stabbr="US")) %>%
...
njus %>%
  ...
  ggtitle("NJ and US year over year growth rates in nominal GDP, state fiscal years",
          subtitle="Moody's July 2020 forecast") +
  theme_bw()
...
njus_diffpch %>%
  ...
  scale_y_continuous(breaks=seq(-20, 20, 1)) +
  scale_x_continuous(breaks=seq(1900, 2030, 5)) +
  ggtitle("NJ minus US year over year growth rates in nominal GDP, state fiscal years",
          subtitle="Moody's July 2020 forecast") +
  theme_bw()

# splice the difference in GDP growth rates on to the gdp forecasts,
# by calculating ratio of each year to the previous year, for US and NJ
usgdp_spliced
njrates <- usgdp_spliced %>%  # 1947+
  filter(year >= start_year) %>%
  select(year, value, src) %>%
  group_by(src) %>%
  mutate(usratio=value / value[match(year - 1, year)]) %>%
  # njus_diff is 1964+
  left_join(njus_diffpch %>%
              select(year, diff) %>%
              mutate(diff=diff / 100),
            by = "year") %>%
  mutate(njratio=ifelse(year==start_year, 1, usratio + diff)) # 1964+

njrates %>%
  select(year, src, njratio) %>%
  pivot_wider(names_from = src, values_from = njratio) %>%
  kable(digits=5, format="rst")


njgdp_spliced <- njrates %>%
  select(year, src, njratio) %>%
  arrange(src, year) %>%
  group_by(src) %>%
  mutate(njratio=cumprod(njratio),
         value=njratio * njsplice$value[njsplice$year == start_year]) %>%
  ungroup() %>%
  select(year, value, src) %>%
  mutate(name="gdp", 
         stabbr="NJ",
...
saveRDS(njgdp_spliced, here::here("data", "njgdp_spliced.rds"))
...
Note: All amounts are in $ millions, nominal, unless noted otherwise.


## NJ taxes vs GDP in history
```{r}
combotab
vars <- c("cit_census", "gst_census", "iit_census", "othertax_census",
          "nontax_census", "osr_census", "gdp_beaspliced")
df <- combo %>%
  filter(fullname %in% vars, stabbr=="NJ", year >= 1964, year <= 2019) %>% # so we always have gdp
  select(year, name, value)
...
dfw <- df %>%
  ...
  group_by(name) %>%
  arrange(year) %>%
  mutate(mapct=ma(pctgdp, n=ma_period)) %>%
  ungroup
count(dfw, name)
# 1 cit         56
# 2 gst         56
...
src <- "osr"
dfw %>%
  select(year, name, pctgdp, mapct) %>%
  pivot_longer(cols=c(pctgdp, mapct), names_to = "var") %>%
  filter(year >= 1978, name==src, !is.na(value)) %>%
  ggplot(aes(year, value, colour=var)) +
  geom_line() +
  geom_point() +
  ggtitle(src)
# allow 3 years for each to return to its pre-recession 5-year average??
...
```{r}
combotab
# history ends in 2020
# apply past recession recovery rates????
# we need to do the following:
#  - get 5-year average revenue as % of state gdp by source, for our long run
#  - get short revenue forecast from govt forecasts
#  - compute path to 5-year average revenue, for each econ forecast

```


### Revenue growth from government forecasts
```{r}
...
### Average revenue as % of gsp
```{r}
pctgdp_years <- 3 # number of years we use for target pct of gdp
...
econdata <- bind_rows(usgdp2, njgdp2)
count(econdata, name)
...
# osrvars <- c("iit", "gst", "cit", "othertax", "nontax", "tottax", "osr") # define the desired order
...
dlong <- d2021 %>%
  ...
  mutate(gap=ifelse(year >= 2015, pctgdp - targetpct, NA_real_),
         years_from_start=case_when(year <= 2021 ~ 0,
                              year  %in% 2022:(2021 + nyears) ~ year - 2021,
                              TRUE ~ 0),
         initial_gap = gap[year==2021],
         initial_pct = pctgdp[year==2021],
         pctgdp_new=case_when(year <= 2021 ~ NA_real_,
                              year %in% 2022:(2021 + nyears) ~ initial_pct - initial_gap * years_from_start / nyears,
                              year > (2021 + nyears) ~ targetpct,
                              TRUE ~ -999),
