...
# readRDS(here::here("data", "usgdphist.rds"))
rm(gdpdata, gdphist)

```


```{r ONETIME_usgdp_forecast, eval=FALSE}
...
rm(mfn, moody, moody2, gdp, gdp2, rgdp, rgdp2, check, stack, sfy)

```


```{r ONETIME_njgdp_history, eval=FALSE}
# year, name, value, stabbr, src, fullname

# bea history from quarterly back to 2005
dfq <- sgdp.q %>%
  filter(stabbr=="NJ") %>%
...
# bea history from annual back to 1963
dfa <- sgdp_spliced.a %>%
  filter(stabbr=="NJ") %>%
  arrange(year, name) %>%
  group_by(name) %>%
...
dfa2 <- dfa %>%
  group_by(src, name) %>%
  mutate(r2005=value / value[year==2005]) %>%
  ungroup
...
saveRDS(dfsave, here::here("data", "njgdp_history.rds"))
...
# df <- read_excel(here::here("ignore", "NJ", fn),
...
#             .groups="drop") %>%
#   filter(year <= 2020)  # 2020 is not true history but should be close
# ht(df3)
# 
# saveRDS(df3, here::here("data", "njgdp_history.rds"))
# rm(fn, df, df2, df3)

```


```{r ONETIME_njgdp_forecast, eval=FALSE}
