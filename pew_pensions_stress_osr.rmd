...
```{r define_recovery_years}
recovery_years <- 2022:2024

trough_year <- min(recovery_years) - 1
recovered_year <- max(recovery_years) + 1
...
recovery_modfc <- revmods %>%
  forecast(new_data=fcast_ts %>% filter(year %in% recovery_years))
...
fcast2 <- fcast %>%
  ...
  mutate(across(c(fcmod, fcgap), ~ ifelse(year > 2021 & revtype=="nontax",
                                          ...
                                          .)),
         across(c(fcmod, fcgap), ~ ifelse(year > max(recovery_years) & revtype!="nontax",
                                          .[year==max(recovery_years)] 
                                          * istgdp / istgdp[year==max(recovery_years)],
                                          .))
         ) %>%
  ungroup
...
# now make a wide file and calculate totals  
