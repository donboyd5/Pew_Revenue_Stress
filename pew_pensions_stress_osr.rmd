...
<!--
Note:
- save all files as nominal, billions, state fiscal year basis
...
## Splice NJ GDP data
```{r}
# get NJ as share of US in the Moody's baseline and assume it carries forward from there
combotab
njshare <- combo %>%
  filter(name=="gdp", src %in% c(""))
...
- put in nominal $ billions
  (include real versions of some variables; and have gdp price index)
- on state fiscal year basis (except capital gains)
...
gdphist <- gdpdata %>%
  ...
  mutate(name=case_when(vname=="A191RC" ~ "gdp",
                        vname=="A191RX" ~ "rgdp",
                        TRUE ~ "ERROR"),
         value=value / 1e3) %>%  # put in $ billions
...
stack <- bind_rows(gdp2, 
                   rgdp2, 
                   moody2 %>% 
                     filter(src == "moodys_jul2020p90", year>=2019)) %>%
  filter(!is.na(value))
...
# don't use BEA because the Moody's history goes back to 1977 but this only goes back to 2005
# df <- sgdp.q %>%
#   filter(stabbr=="NJ") %>%
...
summary(usgdp_history)

usgdp_forecast <- readRDS(here::here("data", "usgdp_forecast.rds"))
summary(usgdp_forecast)

njgdp_history <- readRDS(here::here("data", "njgdp_history.rds"))
summary(njgdp_history)

njgdp_forecast <- readRDS(here::here("data", "njgdp_forecast.rds"))
summary(njgdp_forecast) # 1977-2051 gdp_moodys_dec2019 and gdp_moodys_jul2020, nominal, $ billions
count(njgdp_forecast, fullname)
#   fullname               n
#   <chr>              <int>
